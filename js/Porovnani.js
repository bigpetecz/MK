var Porovnani = {
	...
	init:function(){
		...
		if( !self.inited ) {
			...
			$(".chzn-select").chosen( { no_results_text:"Nic nenalezeno pro: " }); 
	...
	toggleTableRowVisibility:function( filters ){
		...
		if( !this.tableRows ) return;
   		...
   		for( var i = 0 ; i < len; i++ ) {
   			var className = filters[ i ].name;
 			var classRows = this.tableRows.filter( "." + className );	
  			classRows.show();
   		}
	...
	handleSelectChange: function( selectedId ) {
		if( this == Porovnani.firstSelectBox ) Application.updateSelectedUnit( selectedId );
		else Application.updateSecondSelectedUnit( selectedId );
	},
